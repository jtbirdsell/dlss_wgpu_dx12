...
//!
//! ## Documentation
//! This crate is **not published on docs.rs**, and the omission is deliberate: rustdoc on docs.rs
//! cannot build it. The NGX bindings are generated by `bindgen` from NVIDIA's **non-redistributable**
//! DLSS headers — those headers are absent from the docs.rs build sandbox (no `DLSS_SDK`, no network)
//! and cannot be committed as a stub for redistribution reasons, so `build.rs` has no headers to bind.
//! (Independently, the crate pins `wgpu` from a git fork, which blocks crates.io/docs.rs publishing
//! until [gfx-rs/wgpu#9613](https://github.com/gfx-rs/wgpu/pull/9613) lands.) Build the API docs
//! **locally** instead, with the SDK available:
//!
//! ```text
//! cargo doc --open                                          # Super Resolution (default)
//! cargo doc --features ray-reconstruction,frame-generation  # the full surface
//! ```
